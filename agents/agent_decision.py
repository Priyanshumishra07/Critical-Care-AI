...
class AgentConfig:
    ...
    2. RAG_AGENT - For specific medical knowledge questions that can be answered from established medical literature. Currently ingested medical knowledge involves pneumonia diagnosis, skin lesion analysis, and medical imaging protocols.
    ...
    4. PNEUMONIA_AGENT - For analysis of chest X-ray images to detect pneumonia.
    5. SKIN_LESION_AGENT - For segmentation and analysis of skin lesion images in dermatology.
...
def create_agent_graph():
    ...
    # Initialize guardrails with the same LLM used elsewhere (or None if not available)
    guardrails_llm = config.rag.llm if config.rag.llm is not None else None
    guardrails = LocalGuardrails(guardrails_llm) if guardrails_llm is not None else None
    ...
    if decision_model is None:
        print("ERROR: Decision LLM not initialized. Please add API credentials to .env file.")
        # Create a fallback decision function
        def fallback_decision_chain(input_dict):
            input_text = input_dict.get("input", "")
            # Simple rule-based routing when LLM is not available
            if "pneumonia" in input_text.lower() or "chest" in input_text.lower() or "x-ray" in input_text.lower():
                return {"agent": "PNEUMONIA_AGENT", "reasoning": "Detected pneumonia/chest X-ray keywords", "confidence": 0.7}
            elif "skin" in input_text.lower() or "lesion" in input_text.lower() or "dermatology" in input_text.lower():
                return {"agent": "SKIN_LESION_AGENT", "reasoning": "Detected skin lesion keyword", "confidence": 0.7}
            else:
                return {"agent": "CONVERSATION_AGENT", "reasoning": "Default to conversation agent", "confidence": 0.5}
        decision_chain = fallback_decision_chain
    else:
    ...
    def analyze_input(state: AgentState) -> AgentState:
        ...
        # Check input through guardrails if text is present and guardrails available
        if input_text and guardrails is not None:
            try:
                ...
                print(f"Guardrails check failed: {e}, continuing without guardrails")
    ...
    def route_to_agent(state: AgentState) -> Dict:
        ...
        try:
            if decision_model is None:
                # Simple rule-based routing when LLM is not available
                print("Using fallback rule-based routing (LLM not available)")
                if has_image:
                    if "pneumonia" in input_text.lower() or "chest" in input_text.lower() or image_type and "CHEST" in str(image_type):
                        decision = {"agent": "PNEUMONIA_AGENT", "reasoning": "Detected chest X-ray image", "confidence": 0.7}
                    elif "skin" in input_text.lower() or "lesion" in input_text.lower() or "dermatology" in input_text.lower() or image_type and "SKIN" in str(image_type):
                        decision = {"agent": "SKIN_LESION_AGENT", "reasoning": "Detected skin lesion image", "confidence": 0.7}
                    else:
                        decision = {"agent": "PNEUMONIA_AGENT", "reasoning": "Medical image detected, defaulting to pneumonia agent", "confidence": 0.6}
                elif "skin" in input_text.lower() or "lesion" in input_text.lower() or "dermatology" in input_text.lower():
                    decision = {"agent": "SKIN_LESION_AGENT", "reasoning": "Detected skin lesion query", "confidence": 0.7}
                elif "pneumonia" in input_text.lower():
                    decision = {"agent": "PNEUMONIA_AGENT", "reasoning": "Detected pneumonia query", "confidence": 0.7}
                else:
                    decision = {"agent": "CONVERSATION_AGENT", "reasoning": "Default to conversation agent", "confidence": 0.5}
            else:
            ...
            print(f"Error in decision routing: {e}")
            # Fallback to conversation agent
            decision = {"agent": "CONVERSATION_AGENT", "reasoning": f"Error in routing: {str(e)}", "confidence": 0.3}
        ...
        # Only route to needs_validation if using LLM-based routing and confidence is low
        if decision_model is not None and decision["confidence"] < AgentConfig.CONFIDENCE_THRESHOLD:
            return {"agent_state": updated_state, "next": "needs_validation"}
        
        # Return the selected agent directly
    ...
    def run_conversation_agent(state: AgentState) -> AgentState:
        ...
        **You:** "I'm not a doctor, but headaches and fever can have various causes, from infections to dehydration. If your symptoms persist, you should see a medical professional."
        ...
        # Check if LLM is available
        if config.conversation.llm is None:
            response = AIMessage(
                content="I'm sorry, but the AI conversation service is not available. "
                       "Please add your OpenAI API key (e.g., OPENAI_API_KEY=sk-...) to the .env file to enable this feature. "
                       "For now, I can help with:\n"
                       "- Skin lesion segmentation (works without API keys)\n"
                       "- Basic system information\n\n"
                       "To enable full functionality, please:\n"
                       "1. Create or retrieve an API key at https://platform.openai.com/api-keys\n"
                       "2. Add OPENAI_API_KEY=your-key (and optionally EMBEDDING key) to the .env file\n"
    ...
    def run_rag_agent(state: AgentState) -> AgentState:
        ...
        if config.rag.llm is None or config.rag.embedding_model is None:
            response = AIMessage(
                content="**RAG Agent Not Available**\n\n"
                       "The RAG (Retrieval-Augmented Generation) system requires API credentials.\n\n"
                       "To enable RAG:\n"
                       "1. Add Azure OpenAI API keys to .env file\n"
                       "2. (Optional) Ingest medical documents:\n"
                       "   `python ingest_rag_data.py --file ./data/raw/pneumonia_guidelines.pdf`\n"
                       "3. Restart the server\n\n"
                       "For now, you can use the web search agent or skin lesion segmentation."
            )
            return {
                **state,
                "output": response,
    ...
    def run_pneumonia_agent(state: AgentState) -> AgentState:
        """Handle pneumonia detection from chest X-ray images."""
        ...
        predicted_class = AgentConfig.image_analyzer.classify_pneumonia(image_path)

        if predicted_class == "pneumonia":
            response = AIMessage(content="The analysis of the uploaded chest X-ray image indicates a **POSITIVE** result for **PNEUMONIA**. Please consult with a healthcare professional for further evaluation and treatment.")
        elif predicted_class == "normal":
            response = AIMessage(content="The analysis of the uploaded chest X-ray image indicates a **NEGATIVE** result for pneumonia, i.e., **NORMAL** lung appearance.")
        else:
        ...
        return {
            **state,
            ...
            "agent_name": "PNEUMONIA_AGENT"
        }
    
    def run_skin_lesion_agent(state: AgentState) -> AgentState:
        """Handle skin lesion segmentation from dermatology images."""
        ...
        # Create output path for segmentation overlay
        import os
        output_dir = "./uploads/skin_lesion_output"
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, "skin_lesion_segmentation.png")

        # Segment skin lesion
        try:
            success = AgentConfig.image_analyzer.segment_skin_lesion(image_path, output_path)
            
            if success:
                response = AIMessage(
                    content="**Skin Lesion Segmentation Completed**\n\n"
                    "The uploaded skin lesion image has been analyzed and segmented. "
                    "The segmentation overlay has been generated showing the detected lesion boundaries.\n\n"
                    "⚠️ **Important:** This is an AI-assisted analysis. Please consult with a dermatologist "
                    "for professional evaluation and diagnosis."
                )
            else:
                response = AIMessage(content="Unable to perform skin lesion segmentation. Please ensure a valid dermatology image is provided.")
        except Exception as e:
            response = AIMessage(content=f"Error during skin lesion segmentation: {str(e)}")
    ...
    def apply_output_guardrails(state: AgentState) -> AgentState:
        ...
        # Apply output sanitization (if guardrails available)
...
def process_query(query: Union[str, Dict], conversation_history: List[BaseMessage] = None) -> str:
    ...
    # visualize conversation history in console (skip if encoding issues)
    try:
        for m in result["messages"]:
            m.pretty_print()
    except UnicodeEncodeError:
        # Skip pretty printing if encoding issues (Windows console)
        print(f"Conversation history: {len(result['messages'])} messages")
