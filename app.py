...
app = FastAPI(title="Multi-Agent Medical Imaging Diagnostic System", version="2.0", description="Pneumonia & Skin Lesion Detection with Multi-Agent Orchestration")

# Set up directories
UPLOAD_FOLDER = "uploads/backend"
FRONTEND_UPLOAD_FOLDER = "uploads/frontend"
PNEUMONIA_OUTPUT = "uploads/pneumonia_output"
SKIN_LESION_OUTPUT = "uploads/skin_lesion_output"
SPEECH_DIR = "uploads/speech"

# Create directories if they don't exist
for directory in [UPLOAD_FOLDER, FRONTEND_UPLOAD_FOLDER, PNEUMONIA_OUTPUT, SKIN_LESION_OUTPUT, SPEECH_DIR]:
...
def chat(
    ...
    try:
        ...
        # Check for output images from various agents
        result = {
            "status": "success",
        ...
        # Check for output images from various agents
        if "PNEUMONIA_AGENT" in response_data["agent_name"]:
            pneumonia_path = os.path.join(PNEUMONIA_OUTPUT, "pneumonia_analysis.png")
            if os.path.exists(pneumonia_path):
                result["result_image"] = f"/uploads/pneumonia_output/pneumonia_analysis.png"
        elif "SKIN_LESION_AGENT" in response_data["agent_name"]:
            skin_lesion_path = os.path.join(SKIN_LESION_OUTPUT, "skin_lesion_segmentation.png")
            if os.path.exists(skin_lesion_path):
                result["result_image"] = f"/uploads/skin_lesion_output/skin_lesion_segmentation.png"
